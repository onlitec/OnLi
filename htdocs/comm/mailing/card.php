...
if ($action == 'create') {	// aaa
	...
	$doleditor = new DolEditor('bodyemail', GETPOST('bodyemail', 'restricthtmlallowunvalid'), '', 600, 'onli_mailings', '', true, -1, getDolGlobalInt('FCKEDITOR_ENABLE_MAILING'), 20, '100%');
	$doleditor->Create();
	print '</div>';
	...
	if ($object->id > 0) {
		...
		if ($action != 'edit' && $action != 'edittxt' && $action != 'edithtml') {
			...
			if (empty($object->bgcolor) || strtolower($object->bgcolor) == 'ffffff') {	// CKEditor does not apply the color of the div into its content area
				...
				// wysiwyg editor
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor = new DolEditor('bodyemail', $object->body, '', 600, 'onli_mailings', '', false, -1, getDolGlobalInt('FCKEDITOR_ENABLE_MAILING'), 20, '100%', $readonly);
				$doleditor->Create();
			} else {
			...
			*/
			...
			if ($action == 'edit') {
				...
				$doleditor = new DolEditor('bodyemail', $object->body, '', 600, 'onli_mailings', '', true, -1, getDolGlobalInt('FCKEDITOR_ENABLE_MAILING'), 20, '100%');
				$doleditor->Create();
			}
			if ($action == 'edittxt') {
				// wysiwyg editor
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor = new DolEditor('bodyemail', $object->body, '', 600, 'onli_mailings', '', true, -1, 0, 20, '100%');
				$doleditor->Create();
			}
			if ($action == 'edithtml') {
				// HTML source editor
				require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
				$doleditor = new DolEditor('bodyemail', $object->body, '', 600, 'onli_mailings', '', true, -1, 'ace', 20, '100%');
