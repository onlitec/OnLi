...
class Societe extends CommonObject
{
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		// For automatic creation during create action (not used by OnLi GUI, can be used by scripts)
	...
	public function findNearest($rowid = 0, $ref = '', $ref_ext = '', $barcode = '', $idprof1 = '', $idprof2 = '', $idprof3 = '', $idprof4 = '', $idprof5 = '', $idprof6 = '', $email = '', $ref_alias = '', $is_client = 0, $is_supplier = 0)
	{
		...
		dol_syslog(get_class($this)."::findNearest", LOG_DEBUG);
		...
		$sqlstart .= ' WHERE s.entity IN ('.getEntity($this->element).')';
		if ($is_client) {
		...
		$sqlprof = "";
		if ($idprof1) {
			$sqlprof .= " s.siren = '".$this->db->escape($idprof1)."'";
		}
		if ($idprof2) {
			if ($sqlprof) {
				$sqlprof .= " OR";
			}
			$sqlprof .= " s.siret = '".$this->db->escape($idprof2)."'";
		}
		if ($idprof3) {
			if ($sqlprof) {
				$sqlprof .= " OR";
			}
			$sqlprof .= " s.ape = '".$this->db->escape($idprof3)."'";
		}
		if ($idprof4) {
			if ($sqlprof) {
				$sqlprof .= " OR";
			}
			$sqlprof .= " s.idprof4 = '".$this->db->escape($idprof4)."'";
		}
		if ($idprof5) {
			if ($sqlprof) {
				$sqlprof .= " OR";
			}
			$sqlprof .= " s.idprof5 = '".$this->db->escape($idprof5)."'";
		}
		if ($idprof6) {
			if ($sqlprof) {
				$sqlprof .= " OR";
		...
		if ($ref_alias) {
			...
			$sqlref .= " s.name_alias = '".$this->db->escape($ref_alias)."'";
		}
		if ($ref_ext) {
			if ($sqlref) {
				$sqlref .= " OR";
