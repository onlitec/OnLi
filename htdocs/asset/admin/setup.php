...
if ($action == 'updateMask') {
	...
	if ($maskconst && preg_match('/_MASK$/', $maskconst)) {
		$res = onli_set_const($db, $maskconst, $mask, 'chaine', 0, '', $conf->entity);
		if (!($res > 0)) {
			$error++;
		}
	}
					...
					while (($file = readdir($handle)) !== false) {
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php') {
							...
							if ($module->isEnabled()) {
								...
								$defaultifnotset = 'mod_asset_standard';
	...
	foreach ($arrayofparameters as $constname => $val) {
		if ($val['enabled'] == 1) {
			...
			if ($val['type'] == 'textarea') {
				...
				$doleditor = new DolEditor($constname, getDolGlobalString($constname), '', 160, 'onli_notes', '', false, false, isModEnabled('fckeditor'), ROWS_5, '90%');
				$doleditor->Create();
