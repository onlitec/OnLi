...
<tr>
	<td><?php echo $langs->trans("OnLiLogin"); ?></td>
	<td colspan="3"><?php echo $this->control->tpl['onli_user']; ?></td>
</tr>
...
if (empty($user->socid)) {
	...
	if (!$this->control->tpl['user_id'] && $user->hasRight('user', 'user', 'creer')) {
		print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$this->control->tpl['id'].'&action=create_user&token='.newToken().'&canvas='.$canvas.'">'.$langs->trans("CreateOnLiLogin").'</a>';
	}
