...
if (empty($reshook) && $action == 'add') {	// Test on permission not required here. This is an anonymous public ssubmission. Check is done on the secureket + mitigation.
					...
					if (!empty((float) $project->price_booth)) {
						...
						if (!$error && is_object($facture)) {
							...
							/*if (!$error) {
								...
								$reftouse = $facture->id;
								$redirection = $onli_main_url_root.'/public/payment/newpayment.php?source='.$sourcetouse.'&ref='.$reftouse.'&booth='.$conforbooth->id;
	...
	if (!$error && is_object($thirdparty)) {
		...
		$redirection = $onli_main_url_root.'/public/eventorganization/subscriptionok.php?id='.$id.'&securekey='.$securekeyurl;
		header("Location: ".$redirection);
		exit;
	} else {
		$db->rollback();
