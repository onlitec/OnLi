...
class Expedition extends CommonObject
{
	...
	public function valid($user, $notrigger = 0)
	{
		...
		if (!$error && isModEnabled('stock') && getDolGlobalString('STOCK_CALCULATE_ON_SHIPMENT')) {
			$result = $this->manageStockMvtOnEvt($user, "ShipmentValidatedInOnLi");
			if ($result < 0) {
				return -2;
			}
		}
				...
				for ($i = 0; $i < $cpt; $i++) {
					...
					if ($can_update_stock && empty($obj->iskit) && !empty($obj->incdec)) {
						...
						if (empty($lotArray)) {
							...
							$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $obj->qty, 0, $langs->trans("ShipmentCanceledInOnLi", $this->ref), '', '', '', '', 0, '', 0, 1); // Price is set to 0, because we don't want to see WAP changed
							if ($result < 0) {
								$error++;
							...
							foreach ($lotArray as $lot) {
								$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $lot->qty, 0, $langs->trans("ShipmentCanceledInOnLi", $this->ref), $lot->eatby, $lot->sellby, (string) $lot->batch, '', 0, '', 0, 1); // Price is set to 0, because we don't want to see WAP changed
								if ($result < 0) {
									$error++;
									$this->errors = array_merge($this->errors, $mouvS->errors);
									break;
				...
				for ($i = 0; $i < $cpt; $i++) {
					...
					if ($can_update_stock && empty($obj->iskit) && !empty($obj->incdec)) {
						...
						if (empty($lotArray)) {
							...
							// We use warehouse selected for each line
							$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $obj->qty, 0, $langs->trans("ShipmentDeletedInOnLi", $this->ref), '', '', '', '', 0, '', 0, 1); // Price is set to 0, because we don't want to see WAP changed
							if ($result < 0) {
								$error++;
								$this->errors = array_merge($this->errors, $mouvS->errors);
								break;
							...
							// We use warehouse selected for each line
							foreach ($lotArray as $lot) {
								$result = $mouvS->reception($user, $obj->fk_product, $obj->fk_entrepot, $lot->qty, 0, $langs->trans("ShipmentDeletedInOnLi", $this->ref), $lot->eatby, $lot->sellby, (string) $lot->batch, '', 0, '', 0, 1); // Price is set to 0, because we don't want to see WAP changed
								if ($result < 0) {
									$error++;
									$this->errors = array_merge($this->errors, $mouvS->errors);
									break;
	 ...
	 * @throws Exception
	 */
	private function manageStockMvtOnEvt($user, $labelmovement = 'ShipmentClassifyClosedInOnLi')
	{
		global $langs;
				...
				if ($resql) {
					...
					for ($i = 0; $i < $cpt; $i++) {
						...
						if (empty($obj->edbrowid)) {
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result = $mouvS->livraison($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ShipmentUnClassifyCloseddInOnLi", $this->ref));
							if ($result < 0) {
								$this->error = $mouvS->error;
								$this->errors = $mouvS->errors;
								$error++;
							...
							// We decrement stock of product (and sub-products) -> update table llx_product_stock (key of this table is fk_product+fk_entrepot) and add a movement record
							$result = $mouvS->livraison($user, $obj->fk_product, $obj->fk_entrepot, -$qty, $obj->subprice, $langs->trans("ShipmentUnClassifyCloseddInOnLi", $this->ref), '', $this->db->jdate($obj->eatby), $this->db->jdate($obj->sellby), $obj->batch, $obj->fk_origin_stock);
							if ($result < 0) {
								$this->error = $mouvS->error;
								$this->errors = $mouvS->errors;
								$error++;
