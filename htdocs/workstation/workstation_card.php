	...
	*/
	...
	if ($object->type !== 'MACHINE') {
		...
		foreach ($object->usergroups as $id_group) {
			$g->fetch($id_group);
			$toprint[] = '<li class="select2-search-choice-onli noborderoncategories" style="background: #bbb">' . $g->getNomUrl(1, '', 0, 'categtextwhite') . '</li>';
		}
	...
	if ($object->type !== 'HUMAN') {
		...
		foreach ($object->resources as $id_resource) {
			$r->fetch($id_resource);
			$toprint[] = '<li class="select2-search-choice-onli noborderoncategories" style="background: #bbb">' . $r->getNomUrl(1, '', '', 0, 'categtextwhite') . '</li>';
		}
