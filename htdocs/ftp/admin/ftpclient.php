...
if ($action == 'add' || GETPOST('modify', 'alpha')) {
	...
	if (!$error) {
		...
		$result1 = onli_set_const($db, "FTP_PORT_".$entry, GETPOST($ftp_port, 'alpha'), 'chaine', 0, '', $conf->entity);
		if ($result1) {
			$result2 = onli_set_const($db, "FTP_SERVER_".$entry, GETPOST($ftp_server, 'alpha'), 'chaine', 0, '', $conf->entity);
		}
		if ($result2) {
			$result3 = onli_set_const($db, "FTP_USER_".$entry, GETPOST($ftp_user, 'alpha'), 'chaine', 0, '', $conf->entity);
		}
		if ($result3) {
			$result4 = onli_set_const($db, "FTP_PASSWORD_".$entry, GETPOST($ftp_password, 'alpha'), 'chaine', 0, '', $conf->entity);
		}
		if ($result4) {
			$result5 = onli_set_const($db, "FTP_NAME_".$entry, GETPOST($ftp_name, 'alpha'), 'chaine', 0, '', $conf->entity);
		}
		if ($result5) {
			$result6 = onli_set_const($db, "FTP_PASSIVE_".$entry, GETPOST($ftp_passive, 'alpha'), 'chaine', 0, '', $conf->entity);
		}
...
if (GETPOST('delete', 'alpha')) {
	if ($entry) {
		...
		$result1 = onli_del_const($db, "FTP_PORT_".$entry, $conf->entity);
		if ($result1) {
			$result2 = onli_del_const($db, "FTP_SERVER_".$entry, $conf->entity);
		}
		if ($result2) {
			$result3 = onli_del_const($db, "FTP_USER_".$entry, $conf->entity);
		}
		if ($result3) {
			$result4 = onli_del_const($db, "FTP_PASSWORD_".$entry, $conf->entity);
		}
		if ($result4) {
			$result5 = onli_del_const($db, "FTP_NAME_".$entry, $conf->entity);
		}
		if ($result4) {
			$result6 = onli_del_const($db, "FTP_PASSIVE_".$entry, $conf->entity);
		}
