			...
			if ($object->status == $object::STATUS_CONTRACT_SIGNED) {
				if ($user->hasRight('user', 'user', 'creer')) {
					...
					if (empty($useralreadyexists)) {
						print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?id='.$object->id.'&action=create_user&token='.newToken().'">'.$langs->trans("CreateOnLiLogin").'</a></div>';
					} else {
						print '<div class="inline-block divButAction"><a class="butActionRefused" href="#">'.$langs->trans("CreateOnLiLogin").'</a></div>';
					}
				} else {
					print '<div class="inline-block divButAction"><span class="butActionRefused classfortooltip" href="#" title="'.dol_escape_htmltag($langs->trans("NotEnoughPermissions")).'">'.$langs->trans("CreateOnLiLogin")."</span></div>";
				}
			}
