...
function dolSavePageAlias($filealias, $object, $objectpage)
{
	...
	$aliascontent .= 'global $onli_main_data_root;'."\n";
	$aliascontent .= 'if (empty($onli_main_data_root)) $res=include \'./page'.$objectpage->id.'.tpl.php\'; ';
	$aliascontent .= 'else $res=include $onli_main_data_root.\'/website/\'.$website->ref.\'/page'.$objectpage->id.'.tpl.php\';'."\n";
...
function dolSavePageContent($filetpl, Website $object, WebsitePage $objectpage, $backupold = 0)
{
	...
	if (!isset($originalcontentonly)) {
		...
		$tplcontent .= '<meta name="generator" content="'.DOL_APPLICATION_TITLE.' '.DOL_VERSION.' (https://www.onli.org)" />'."\n";
		$tplcontent .= '<meta name="onli:pageid" content="'.((int) $objectpage->id).'" />'."\n";
		...
		// Page content
		$tplcontent .= '<!-- File content defined in OnLi website module editor -->'."\n";
...
function checkPHPCode(&$phpfullcodestringold, &$phpfullcodestring)
{
	...
	if (!$error) {
		...
		if (!getDolGlobalString('WEBSITE_PHP_ALLOW_EXEC')) {    // If option is not on, we disallow functions to execute commands
			...
			$forbiddenphpfunctions = array_merge($forbiddenphpfunctions, array("dol_eval", "executeCLI", "verifCond"));	// native onli functions
