...
function dolSessionOpen($save_path, $session_name)
{
	...
	global $onli_main_db_type, $onli_main_db_host;
	global $onli_main_db_user, $onli_main_db_pass, $onli_main_db_name, $onli_main_db_port;

	global $onli_session_db_type, $onli_session_db_host;
	global $onli_session_db_user, $onli_session_db_pass, $onli_session_db_name, $onli_session_db_port;

	if (empty($onli_session_db_type)) {
		$onli_session_db_type = $onli_main_db_type;
	}
	if (empty($onli_session_db_host)) {
		$onli_session_db_host = $onli_main_db_host;
	}
	if (empty($onli_session_db_user)) {
		$onli_session_db_user = $onli_main_db_user;
	}
	if (empty($onli_session_db_pass)) {
		$onli_session_db_pass = $onli_main_db_pass;
	}
	if (empty($onli_session_db_name)) {
		$onli_session_db_name = $onli_main_db_name;
	}
	if (empty($onli_session_db_port)) {
		$onli_session_db_port = $onli_main_db_port;
	}
	//var_dump('open '.$database_name.' '.$table_name);

	$dbsession = getDoliDBInstance($onli_session_db_type, $onli_session_db_host, $onli_session_db_user, $onli_session_db_pass, $onli_session_db_name, (int) $onli_session_db_port);
