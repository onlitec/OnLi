...
/**
 *	\file       htdocs/core/modules/accountancy/doc/pdf_bookkeeping.modules.php
 *	\ingroup    accountancy
 *	\brief      Class file allowing accountancy bookkeeping template generation
 */
 ...
 */
class pdf_bookkeeping extends ModelePdfAccountancy
{
	/**
	...
	public function __construct(DoliDB $db)
	{
		...
		$this->description = $langs->trans("PDFAccountancyBookkeepingDescription");
	...
	public function write_file(BookKeeping $object, Translate $outputlangs, string $srctemplatepath = '', bool $directDownload = true)
	{
		...
		$journal = '';
		$journalDebit = $journalCredit = $totalDebit = $totalCredit = 0;
		for ($i = 0; $i < $nblines; $i++) {
			// Show total line / title line when account has changed
			if (empty($journal) || $journal != $object->lines[$i]->code_journal) {
				...
				$journal = $object->lines[$i]->code_journal;
				$journalDebit = $journalCredit = 0;
			}

			$journalDebit += $object->lines[$i]->debit;
			$journalCredit += $object->lines[$i]->credit;
			...
			if ($this->getColumnStatus('account')) {
				$this->printStdColumnContent($pdf, $curY, 'account', length_accountg($object->lines[$i]->numero_compte));
				$nexY = max($pdf->GetY(), $nexY);
			}
			if ($this->getColumnStatus('account_label')) {
				$this->printStdColumnContent($pdf, $curY, 'account_label', $object->lines[$i]->label_compte);
				$nexY = max($pdf->GetY(), $nexY);
			}
	...
	protected function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetTextColor(0, 0, 60);
		$title = $outputlangs->transnoentities("PdfBookkeepingTitle");
