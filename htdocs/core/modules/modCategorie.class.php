 ...
 */
class modCategorie extends OnLiModules
{
	/**
	...
	public function __construct($db)
	{
		...
		if (isModEnabled("product")) {
			$categcode = 'product';
			$r++;
			$this->export_code[$r] = $this->rights_class.'_0_'.$categcode;
		...
		if (isModEnabled("societe")) {
			$categcode = 'supplier';
			$r++;
			$this->export_code[$r] = $this->rights_class.'_1_'.$categcode;
		...
		if (isModEnabled("societe")) {
			$categcode = 'customer';
			$r++;
			$this->export_code[$r] = $this->rights_class.'_2_'.$categcode;
		...
		if (isModEnabled("member")) {
			$categcode = 'member';
			$r++;
			$this->export_code[$r] = $this->rights_class.'_3_'.$categcode;
		...
		if (isModEnabled("project")) {
			$categcode = 'project';
			$r++;
			$this->export_code[$r] = $this->rights_class.'_6_'.$categcode;
		...
		if (isModEnabled("user")) {
			$categcode = 'user';
			$r++;
			$this->export_code[$r] = $this->rights_class.'_7_'.$categcode;
		...
		if (isModEnabled("product")) {
			$categcode = 'product';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_0_'.$categcode;
		...
		if (isModEnabled("societe")) {
			$categcode = 'supplier';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_1_'.$categcode;
		...
		if (isModEnabled("societe")) {
			$categcode = 'customer';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_2_'.$categcode;
		...
		if (isModEnabled('member')) {
			$categcode = 'member';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_3_'.$categcode;
		...
		if (isModEnabled("societe")) {
			$categcode = 'contact';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_4_'.$categcode;
		...
		if (isModEnabled('project')) {
			$categcode = 'project';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_6_'.$categcode;
		...
		if (isModEnabled('user')) {
			$categcode = 'user';
			$r++;
			$this->import_code[$r] = $this->rights_class.'_7_'.$categcode;
	...
	protected function exportTagLinks(int $r, string $categcode, string $elementtype, string $enabled, array $permission, array $fields_list)
	{
		...
		$this->export_code[$r] = $this->rights_class.'_'.$cat_id.'_'.$categcode;
		$this->export_label[$r] = 'Cat'.ucfirst($categcode).'sLinks';
		$this->export_icon[$r] = $this->picto;
	...
	protected function importTagLinks(int $r, string $categcode, string $class_file, string $elementtype, string $element)
	{
		...
		$this->import_tables_array[$r] = ['ci' => MAIN_DB_PREFIX.'categorie_'.$categcode];
