	...
	public function showRoles($htmlname, Contact $contact, $rendermode = 'view', $selected = array(), $morecss = 'minwidth500', $placeholder = '')
	{
		if ($rendermode === 'view') {
			...
			foreach ($contact->roles as $key => $val) {
				$toprint[] = '<li class="select2-search-choice-onli noborderoncategories" style="background: #bbb;">' . $val['label'] . '</li>';
			}
			return '<div class="select2-container-multi-onli" style="width: 90%;" id="' . $htmlname . '"><ul class="select2-choices-onli">' . implode(' ', $toprint) . '</ul></div>';
		}
