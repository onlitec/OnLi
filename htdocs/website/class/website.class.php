...
class Website extends CommonObject
{
	...
	public function create(User $user, $notrigger = 0)
	{
		...
		if (!$error) {
			$stringtoonlifile = "# Some properties for OnLi web site CMS\n";
			$stringtoonlifile .= "param=value\n";
			//print $conf->website->dir_output.'/'.$this->ref.'/.onli';exit;
			file_put_contents($conf->website->dir_output.'/'.$this->ref.'/.onli', $stringtoonlifile);
		}
	...
	public function createFromClone($user, $fromid, $newref, $newlang = '')
	{
		...
		$pathofwebsiteold = $onli_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($oldref);
		$pathofwebsitenew = $onli_main_data_root.($conf->entity > 1 ? '/'.$conf->entity : '').'/website/'.dol_sanitizeFileName($newref);
	...
	public function exportWebSite()
	{
		...
		$line = '-- File generated by OnLi '.DOL_VERSION.' --;'."\n";
		$line .= "\n";
		fwrite($fp, $line);
	...
	protected function showDifferences($str1, $str2, $exceptNumPge = array())
	{
		...
		if (empty($linesShouldChange)) {
			$linesShouldChange[0] = '<meta name="onli:pageid" content="'.$exceptNumPge[0].'" />';
