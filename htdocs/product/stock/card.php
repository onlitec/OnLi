...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('desc', (!empty($object->description) ? $object->description : ''), '', 180, 'onli_notes', 'In', false, true, isModEnabled('fckeditor'), ROWS_5, '90%');
	$doleditor->Create();
	print '</td></tr>';
	...
	if ($id > 0 || $ref) {
			...
			if (isModEnabled('project') && $formproject !== null) {
				...
				if ($usercancreate) {
					...
					if ($action == 'classify') {
						...
						$morehtmlref .= $form->form_project($_SERVER['PHP_SELF'].'?id='.$object->id, ($socid > 0 ? $socid : -1), (string) $object->fk_project, 'none', 0, 0, 0, 1, '', 'maxwidth300');
					}
				} else {
		...
		if ($action == 'edit' || $action == 're-edit') {
			...
			// Editeur wysiwyg
			require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
			$doleditor = new DolEditor('desc', $object->description, '', 180, 'onli_notes', 'In', false, true, isModEnabled('fckeditor'), ROWS_5, '90%');
			$doleditor->Create();
			print '</td></tr>';
...
if ($action != 'create' && $action != 'edit' && $action != 'delete') {
	...
	print $formfile->showdocuments($modulepart, $objectref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 0, 0, 0, 28, 0, '', '', '', '', '', $object);
