...
class AccountancyExport
{
	...
	public function exportCiel($objectLines, $exportFile = null)
	{
		...
		foreach ($objectLines as $line) {
			...
			$tab[] = str_repeat(' ', 18); // Analytical accounting - Not managed in OnLi
	 ...
	 * Help to import in Quadra: https://wiki.onli.org/index.php?title=Module_Comptabilit%C3%A9_en_Partie_Double#Import_vers_CEGID_Quadra
					...
					if (!empty($fileFoundList)) {
						...
						foreach ($fileFoundList as $fileFound) {
							if (strstr($fileFound['name'], $objectFileName)) {
								...
								// We want to retrieve an attachment representative of the supplier invoice, not a fake document generated by OnLi.
	 ...
	 * Last review for this format : 2023/10/12 Alexandre Spangaro (aspangaro@open-dsi.fr)
	 *
	 * Help to import in your software: https://wiki.onli.org/index.php?title=Module_Comptabilit%C3%A9_en_Partie_Double#Exports_avec_fichiers_sources
	 *
	 * @param 	BookKeepingLine[]	$objectLines 			data
	 * @param	?resource	$exportFile				[=null] File resource to export or print if null
	 * @param 	array<string,array{name:string,path:string}>		$archiveFileList		[=array()] Archive file list : array of ['path', 'name']
