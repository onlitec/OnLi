...
if ($action == 'create') {
	...
	$doleditor = new DolEditor('comment', (GETPOSTISSET('comment') ? GETPOST('comment', 'restricthtml') : $object->note_public), '', 200, 'onli_notes', '', false, true, isModEnabled('fckeditor'), 15, '90%');
	$doleditor->Create();

	print '<tr><td class="tdtop">'.$langs->trans("WelcomeEMail").'</td><td>';
	require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
	$doleditor = new DolEditor('mail_valid', GETPOSTISSET('mail_valid') ? GETPOST('mail_valid') : $object->mail_valid, '', 250, 'onli_notes', '', false, true, isModEnabled('fckeditor'), 15, '90%');
	$doleditor->Create();
	print '</td></tr>';
...
if ($rowid > 0) {
	...
	if ($action == 'edit') {
		...
		print '<tr><td class="tdtop">'.$langs->trans("Description").'</td><td>';
		require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
		$doleditor = new DolEditor('comment', $object->note_public, '', 220, 'onli_notes', '', false, true, isModEnabled('fckeditor'), 15, '90%');
		$doleditor->Create();
		print "</td></tr>";

		print '<tr><td class="tdtop">'.$langs->trans("WelcomeEMail").'</td><td>';
		$doleditor = new DolEditor('mail_valid', $object->mail_valid, '', 280, 'onli_notes', '', false, true, isModEnabled('fckeditor'), 15, '90%');
		$doleditor->Create();
		print "</td></tr>";
