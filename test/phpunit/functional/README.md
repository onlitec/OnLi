...
Any web server compatible with OnLi will do.

For the full test, it should be configured for serving OnLi's htdocs directory at `https://dev.onli.org` with SSL/TLS enabled.
...
The database used for the test is `onli_test`.

**WARNING:**  
This database will be dropped before and after the test.  
Make sure you don't hold any valuable information in it!

A user called `onli` with a password `onli` will be created as part of the test.
...
Then from OnLi's root directory, run:
